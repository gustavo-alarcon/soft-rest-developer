...
@Component({
  selector: 'app-purchases-create-provider-dialog',
  templateUrl: './purchases-create-provider-dialog.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PurchasesCreateProviderDialogComponent implements OnInit {
  ...
  ruc = new BehaviorSubject(false);
  ruc$ = this.ruc.asObservable();
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<PurchasesCreateProviderDialogComponent>,
  ...
  ngOnInit() {
    ...
    this.rucLoading$ = combineLatest(
      this.dbs.providers$,
      this.dataFormGroup.get('ruc').valueChanges.pipe(tap(() => { this.ruc.next(true) }), debounceTime(300))
    ).pipe(
      map(([providers, ruc]) => {
